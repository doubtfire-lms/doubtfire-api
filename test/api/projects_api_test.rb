...
class ProjectsApiTest < ActiveSupport::TestCase
  ...
  def test_projects_returns_correct_data
    ...
    keys = %w[id unit campus_id user_id target_grade portfolio_available portfolio_locked spec_con_days escalation_attempts_remaining]
  ...
  def test_get_project_response_is_correct
    ...
    keys = %w[id unit unit_id user_id campus_id target_grade submitted_grade portfolio_files compile_portfolio portfolio_available portfolio_locked uses_draft_learning_summary tasks tutorial_enrolments groups spec_con_days escalation_attempts_remaining]
    key_test = keys - %w[unit user_id portfolio_available portfolio_locked tasks tutorial_enrolments groups]
  ...
  def test_submitted_grade_cant_change_after_submission
    ...
    keys = %w[campus_id target_grade submitted_grade compile_portfolio portfolio_available portfolio_locked uses_draft_learning_summary]

    assert_json_limit_keys_to_exactly keys, last_response_body
    assert_json_matches_model project, last_response_body, keys - %w[portfolio_locked]
