...
class StatusTest < ActiveSupport::TestCase
  ...
  def test_status_comments
    ...
    # Task status generated by students is read by the author but remains unread for staff.
    assert rff_comment.read_by?(user), 'Error: RFF status comment should be read by the student'
    assert_not rff_comment.read_by?(tutor), 'Error: RFF status comment should remain unread for the tutor'

    # Task status comments by staff remain unread for students until comments are opened.
    assert te_comment.read_by?(tutor), 'Error: TE status comment should be read by the tutor'
    assert_not te_comment.read_by?(user), 'Error: TE status comment should remain unread for the student'
