...
class TaskTest < ActiveSupport::TestCase
  ...
  def test_trigger_transition_requires_manual_feedback_before_assessment_outcomes
    project = FactoryBot.create(:project)
    unit = project.unit
    task_definition = unit.task_definitions.first
    task = project.task_for_task_definition(task_definition)
    ...
    assert_nil task.trigger_transition(trigger: 'complete', by_user: tutor)
    ...
    assert_nil task.trigger_transition(trigger: 'fix', by_user: tutor)
    assert_equal TaskStatus.ready_for_feedback, task.task_status

    task.reload
    task.add_text_comment(tutor, '**Automated Message:** Automated feedback is not enough')

    assert_nil task.trigger_transition(trigger: 'redo', by_user: tutor)
    assert_equal TaskStatus.ready_for_feedback, task.task_status
