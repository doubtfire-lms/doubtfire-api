...
def test_staff_assigned_statuses
  ...
  data_to_post = {
    ...
    # Get the task... check it is now time exceeded
  task = project.task_for_task_definition(td) 
  assert_equal TaskStatus.find(task.task_status.id), task.task_status 
end  
...
def test_status_key
  ...
  td = TaskDefinition.new({
        ...
        target_grade: 0,
        start_date: Time.zone.now + 2.weeks,
        target_date: Time.zone.now + 3.week,
        due_date: Time.zone.now + 1.week,
        abbreviation: 'UpcomingTask',
        restrict_status_updates: false,
        upload_requirements: [ ],
        plagiarism_warn_pct: 0.8,
        is_graded: false,
  ...
  assert_equal status.status_key, :not_started
end 
