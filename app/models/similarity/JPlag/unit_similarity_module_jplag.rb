...
module UnitSimilarityModuleJPLAG
  #
  ...
  def check_jplag_similarity(force: false)
    ...
    begin
      ...
      task_definitions.each do |td|
        ...
        #   add_done_files_for_plagiarism_check_of(td, submissions_path, to_check, tasks_with_files)
#
        #   FileUtils.chdir(submissions_path)
#
...
#
        ...
        # ensure
        #   FileUtils.chdir(pwd)
        #   FileUtils.rm_rf submissions_path
        # end
      end
  ...
  #
  def add_done_files_for_plagiarism_check_of(task_definition, submissions_path, to_check, tasks_with_files)
    ...
    task_definition.upload_requirements.each_with_index do |upreq, idx|
      ...
      tasks_with_files.each do |t|
        t.extract_file_from_done(submissions_path, pattern, ->(_task, to_path, name) { File.join(to_path.to_s, t.student.username.to_s, name.to_s) })
      end
