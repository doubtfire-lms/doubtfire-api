...
class TaskSubmissionPdfsDownloadAuthorizationsController < ApplicationController
  include AuthenticationHelpers
  include AuthorisationHelpers
  include TaskSubmissionPdfsDownloadAuthentication

  skip_forgery_protection only: :create

  DOWNLOAD_PATH = %r{\A/api/submission/unit/(?<unit_id>\d+)/task_definitions/(?<task_definition_id>\d+)/student_pdfs(?:\?.*)?\z}
  ...
  def show
    ...
    @download_user = authenticated_task_submission_pdfs_download_user(
  ...
  def create
    @download_user = authenticated_task_submission_pdfs_header_user
    return head :unauthorized unless @download_user
  ...
  def download_url_path(unit, task_definition)
    "/api/submission/unit/#{unit.id}/task_definitions/#{task_definition.id}/student_pdfs"
  end

  def download_filename(unit, task_definition, user)
    download_id = "#{Time.zone.now.strftime('%Y-%m-%d %H:%m:%S')}-#{unit.code}-#{task_definition.abbreviation}-#{user.username}-pdfs"
