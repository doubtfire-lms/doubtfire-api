...
class DownloadSubmissionFilesJob
  include Sidekiq::Job
  ...
  def perform(current_user_id, task_definition_id, unit_id)
    ...
    output_zip = unit.get_task_submissions_zip(current_user, td, progress_callback: lambda { |message: nil, total_rows: nil, rows_processed: nil|
    ...
    logger.info "Compressed submission files download!"
