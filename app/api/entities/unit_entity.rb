module Entities
  class UnitEntity < Grape::Entity
    ...
    expose :discuss_timeout_enabled, unless: :summary_only, if: lambda { |unit, options| is_staff?(options[:my_role]) }
    expose :discuss_timeout_warning_days, unless: :summary_only, if: lambda { |unit, options| is_staff?(options[:my_role]) }
    expose :discuss_timeout_expire_days, unless: :summary_only, if: lambda { |unit, options| is_staff?(options[:my_role]) }
